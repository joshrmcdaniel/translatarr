...
- **排序的翻译选项** — 每次请求返回 2–3 个自然的翻译，最佳选项排在最前，每个选项都标注了语体（敬语/中性/口语）
...
系统提示词同样可编辑（按用户或实例级别），并支持 `{{source}}`/`{{target}}` 占位符；JSON 输出约定由服务器端自动追加，确保响应始终可被解析。
...
- 按 **Enter**（或点击发送）将该轮对话保存到当前聊天中。
- 翻译方向在语言对内自动判断：用任意一种语言输入，都会翻译成另一种语言。
- 点击翻译卡片上的备选选项可将其设为主选项；关键词对照表会跟随所选的选项更新。
