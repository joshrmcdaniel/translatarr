...
- `GET|PUT /api/settings` — per-user prefs (model/systemPrompt/speechEngine) + effective view; admins also see the instance section. `PUT /api/settings/instance` — admin-only instance overrides (LLM + `speech*` fields). PUT semantics per field: omitted = unchanged, `null`/empty = clear, value = set.
