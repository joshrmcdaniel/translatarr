"""Synchronous Translatarr API client."""
...
class TranslatarrClient:
    """A synchronous client for a Translatarr instance.

    Authenticate with a personal API key (`Authorization: Bearer <token>`, minted
    under Settings → API keys) or, for a browser-derived session, the
    `translatarr_session` cookie value. At least one of `token` or
    ...
    def __enter__(self) -> TranslatarrClient:
        return self
    ...
    def _send(
        ...
        *,
        ...
        response = self._client.request(method, path, json=json, params=params, data=data, files=files)
    ...
    def list_chats(self) -> list[ChatSummary]:
        ...
        return core.read_chats(self._send("GET", "/api/chats").json())
    ...
    def create_chat(
        ...
        *,
        ...
        return core.read_chat(self._send("POST", "/api/chats", json=body).json())

    def get_chat(self, chat_id: str, *, limit: int | None = None) -> ChatDetail:
        ...
        return core.read_chat(self._send("GET", f"/api/chats/{chat_id}", params=params).json())
    ...
    def list_turns(
        ...
        *,
        ...
        return core.read_turn_page(self._send("GET", f"/api/chats/{chat_id}/turns", params=params).json())

    def rename_chat(self, chat_id: str, *, title: str) -> ChatDetail:
        """Rename a chat."""
        body = core.rename_chat_body(title)
        return core.read_chat(self._send("PATCH", f"/api/chats/{chat_id}", json=body).json())

    def clear_chat(self, chat_id: str) -> ChatDetail:
        """Delete every turn in a chat, keeping the chat itself."""
        return core.read_chat(self._send("PATCH", f"/api/chats/{chat_id}", json=core.CLEAR_CHAT_BODY).json())

    def delete_chat(self, chat_id: str) -> None:
        """Delete a chat and all of its turns."""
        self._send("DELETE", f"/api/chats/{chat_id}")
    ...
    def switch_branch(self, chat_id: str, turn_id: str) -> ChatDetail:
        ...
        return core.read_chat(
            self._send("PATCH", f"/api/chats/{chat_id}/turns/{turn_id}", json=core.SWITCH_BRANCH_BODY).json()
        )
    ...
    def transcribe(
        ...
        *,
        ...
        return core.read_text(self._send("POST", "/api/speech/transcribe", files=files, data=data).json())

    def synthesize(self, text: str, *, lang: TargetLang, voice: str | None = None) -> bytes:
        """Synthesize `text` to MP3 audio bytes via the speech provider."""
        body = core.synthesize_body(text, lang, voice)
        return self._send("POST", "/api/speech/synthesize", json=body).content

    # --- API keys --------------------------------------------------------

    def list_keys(self) -> list[ApiKey]:
        """List the caller's API keys (metadata only, never the secrets)."""
        return core.read_keys(self._send("GET", "/api/keys").json())

    def create_key(self, name: str, *, expires_at: str | datetime | None = None) -> CreatedApiKey:
        """Mint an API key. The plaintext token is returned only here, once."""
        body = core.create_key_body(name, expires_at)
        return core.read_created_key(self._send("POST", "/api/keys", json=body).json())

    def revoke_key(self, key_id: str) -> None:
        """Revoke one of the caller's API keys."""
        self._send("DELETE", f"/api/keys/{key_id}")
